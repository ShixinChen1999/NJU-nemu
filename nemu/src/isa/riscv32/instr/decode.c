...
static def_DHelper(J){//add new decode for J
...
static def_DHelper(B){//add new decode for B
    sword_t offset= ((s->isa.instr.b.imm11 << 11) | (s->isa.instr.b.imm4_1 << 1) \
                    | (s->isa.instr.b.imm10_5 << 5) | (s->isa.instr.b.imm12 << 12))\
                                                           <<20>>20;//sign extended
...
def_THelper(jalr_branch) {
  def_INSTR_TAB("??????? ????? ????? 000 ????? ????? ??", jalr);

  return EXEC_ID_inv;
}

def_THelper(addi_branch) {
...
def_THelper(main) {
  ...
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00100 11", I     , addi_branch);
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 01100 11", R     , add_branch);
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00101 11", U     , auipc);
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 11001 11", I     , jalr_branch);
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 11000 11", B     , B_branch);
